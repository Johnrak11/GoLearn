...
export async function updateCourseProgressService(params: {
  userId: string;
  courseId: string;
  lesson_id: string;
  is_completed?: boolean;
  ...
  const lessonCourseId = (lesson as any).module.course_id as string;
  if (lessonCourseId !== params.courseId) {
    return { error: "Lesson does not belong to the provided course" } as any;
  }
  ...
  const completed = params.is_completed ?? true;
  ...
  const totalLessons = await prisma.lesson.count({
    where: { module: { course_id: params.courseId } },
  });
  const completedLessons = await prisma.lessonProgress.count({
    where: { enrollment_id: enrollment.id, is_completed: true },
  });
